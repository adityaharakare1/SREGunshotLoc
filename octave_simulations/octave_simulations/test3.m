...
h_matched_muzzle = flipud(conj(xt_muzzle))/norm(xt_muzzle);
...
R_target=90;					% target range (range is considered from reference)
...
b=upsample((1-decay).^(0:Necho)',D+1);		% reverb filter
...
for(k1=1:N)
	...
	x_muzzle(k1,:) = filter(b,1,filter(xt_muzzle,1,h_muzzle)') + muzzle_noise;
end
x_received = x_muzzle;
