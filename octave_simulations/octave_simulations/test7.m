% the signal does not reach all sensors at the exact delay required. there
% is always some jitter associated. let's see how the jitter in delay affects
% the range estimate.
...
for(k3=1:numel(Nvec))
	...
	for(k1=1:Nelements)
		...
		jitter_noise = (2*rand - 1)*jitter_dev*1e-3;
		h_muzzle = A/sqrt(4*pi*dR(k1)^2)*sinc(fc*(t - (tau_muzzle(k1)+jitter_noise)));
...
plot(Nvec,err_loc_estimate); grid on;
xlabel('Number of sensors'); ylabel('Range error (m)');
